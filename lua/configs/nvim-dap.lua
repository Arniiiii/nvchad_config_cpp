...
dap.adapters.python = function(cb, config)
  if config.request == "attach" then
    ...
    cb {
      type = "executable",
      command = mason_registry.get_package("debugpy"):get_install_path() .. "/debugpy-adapter",
...
dap.configurations.rust = {
  {
    ...
    request = "launch",
    program = function()
      return vim.fn.getcwd() .. "/target/debug/lrc20d"
    end,
    args = {"run", "--config", vim.fn.getcwd() .. "/config.toml"} ,
    cwd = "${workspaceFolder}",
    stopOnEntry = false,
  },
}
